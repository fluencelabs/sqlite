...
void *sqlite3_value_pointer(sqlite3_value *pVal, const char *zPType) {
  ...
  if ((p->flags & (MEM_TypeMask | MEM_Term | MEM_Subtype)) ==
      ...
      zPType != 0 && p->eSubtype == 'p' && strcmp(p->u.zPType, zPType) == 0) {
    return (void *)p->z;
  } else {
    return 0;
  }
...
*/
static void setResultStrOrError(sqlite3_context *pCtx, /* Function context */
...
void sqlite3_result_blob(sqlite3_context *pCtx, const void *z, int n,
                         void (*xDel)(void *)) {
  assert(n >= 0);
  assert(sqlite3_mutex_held(pCtx->pOut->db->mutex));
  setResultStrOrError(pCtx, z, n, 0, xDel);
}
void sqlite3_result_blob64(sqlite3_context *pCtx, const void *z,
...
void sqlite3_result_pointer(sqlite3_context *pCtx, void *pPtr,
                            const char *zPType, void (*xDestructor)(void *)) {
  Mem *pOut = pCtx->pOut;
...
void sqlite3_result_text(sqlite3_context *pCtx, const char *z, int n,
                         void (*xDel)(void *)) {
  assert(sqlite3_mutex_held(pCtx->pOut->db->mutex));
  setResultStrOrError(pCtx, z, n, SQLITE_UTF8, xDel);
}
void sqlite3_result_text64(sqlite3_context *pCtx, const char *z,
                           sqlite3_uint64 n, void (*xDel)(void *),
                           unsigned char enc) {
  assert(sqlite3_mutex_held(pCtx->pOut->db->mutex));
  assert(xDel != SQLITE_DYNAMIC);
...
#ifndef SQLITE_OMIT_UTF16
void sqlite3_result_text16(sqlite3_context *pCtx, const void *z, int n,
                           void (*xDel)(void *)) {
  assert(sqlite3_mutex_held(pCtx->pOut->db->mutex));
  setResultStrOrError(pCtx, z, n, SQLITE_UTF16NATIVE, xDel);
}
void sqlite3_result_text16be(sqlite3_context *pCtx, const void *z, int n,
                             void (*xDel)(void *)) {
  assert(sqlite3_mutex_held(pCtx->pOut->db->mutex));
  setResultStrOrError(pCtx, z, n, SQLITE_UTF16BE, xDel);
}
void sqlite3_result_text16le(sqlite3_context *pCtx, const void *z, int n,
                             void (*xDel)(void *)) {
  assert(sqlite3_mutex_held(pCtx->pOut->db->mutex));
...
void sqlite3_result_error_toobig(sqlite3_context *pCtx) {
  ...
  sqlite3VdbeMemSetStr(pCtx->pOut, "string or blob too big", -1, SQLITE_UTF8,
                       SQLITE_STATIC);
}
...
end_of_step:
  ...
  ** statements prepared using the legacy sqlite3_prepare() interface */
  assert((p->prepFlags & SQLITE_PREPARE_SAVESQL) != 0 || rc == SQLITE_ROW ||
         rc == SQLITE_DONE || rc == SQLITE_ERROR ||
...
void *sqlite3_get_auxdata(sqlite3_context *pCtx, int iArg) {
  ...
  for (pAuxData = pCtx->pVdbe->pAuxData; pAuxData;
    ...
    if (pAuxData->iAuxArg == iArg &&
        (pAuxData->iAuxOp == pCtx->iOp || iArg < 0)) {
...
** access code.
*/
void sqlite3_set_auxdata(sqlite3_context *pCtx, int iArg, void *pAux,
                         void (*xDelete)(void *)) {
  AuxData *pAuxData;
...
** in the result set.
*/
void sqlite3_column_blob_(sqlite3_stmt *pStmt, int i) __attribute__((export_name("sqlite3_column_blob"))) {
...
void sqlite3_column_text_(sqlite3_stmt *pStmt, int i)
__attribute__((export_name("sqlite3_column_text"))) {
...
*/
static const void *
columnName(sqlite3_stmt *pStmt, /* The statement */
...
void sqlite3_column_name_(sqlite3_stmt *pStmt, int N)
__attribute__((export_name("sqlite3_column_name"))) {
...
static int vdbeUnbind(Vdbe *p, unsigned int i) {
  ...
  if (p->eVdbeState != VDBE_READY_STATE) {
    ...
    sqlite3_mutex_leave(p->db->mutex);
    sqlite3_log(SQLITE_MISUSE, "bind on a busy prepared statement: [%s]",
                p->zSql);
    return SQLITE_MISUSE_BKPT;
  }
...
*/
static int bindText(sqlite3_stmt *pStmt, /* The statement to bind against */
...
int sqlite3_bind_pointer(sqlite3_stmt *pStmt, int i, void *pPtr,
                         const char *zPTtype, void (*xDestructor)(void *)) {
  int rc;
  Vdbe *p = (Vdbe *)pStmt;
  rc = vdbeUnbind(p, (u32)(i - 1));
  if (rc == SQLITE_OK) {
...
#ifndef SQLITE_OMIT_UTF16
int sqlite3_bind_text16(sqlite3_stmt *pStmt, int i, const void *zData,
                        int nData, void (*xDel)(void *)) {
  return bindText(pStmt, i, zData, nData, xDel, SQLITE_UTF16NATIVE);
}
#endif /* SQLITE_OMIT_UTF16 */
int sqlite3_bind_value(sqlite3_stmt *pStmt, int i,
  ...
  case SQLITE_FLOAT: {
    ...
    rc = sqlite3_bind_double(pStmt, i,
                             (pValue->flags & MEM_Real) ? pValue->u.r
                                                        : (double)pValue->u.i);
    break;
  }
  case SQLITE_BLOB: {
  ...
  case SQLITE_TEXT: {
    rc =
        bindText(pStmt, i, pValue->z, pValue->n, SQLITE_TRANSIENT, pValue->enc);
    break;
  }
  default: {
...
int sqlite3_transfer_bindings(sqlite3_stmt *pFromStmt, sqlite3_stmt *pToStmt) {
  ...
  assert((pFrom->prepFlags & SQLITE_PREPARE_SAVESQL) != 0 ||
         pFrom->expmask == 0);
  if (pFrom->expmask) {
    pFrom->expired = 1;
  }
...
int sqlite3_stmt_status(sqlite3_stmt *pStmt, int op, int resetFlag) {
  ...
  u32 v;
#ifdef SQLITE_ENABLE_API_ARMOR
  if (!pStmt || (op != SQLITE_STMTSTATUS_MEMUSED &&
                 (op < 0 || op >= ArraySize(pVdbe->aCounter)))) {
    (void)SQLITE_MISUSE_BKPT;
    return 0;
  }
#endif
...
*/
static UnpackedRecord *vdbeUnpackRecord(KeyInfo *pKeyInfo, int nKey,
                                        const void *pKey) {
